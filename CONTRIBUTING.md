...
- **JS → Swift**: [EditorBridgeMessages.swift](Sources/NotepadX/Features/Editor/RichEditor/EditorBridgeMessages.swift)의 `IncomingBridgeMessageType` (`ready`/`docChanged`/`selectionChanged`/`openExternalLink`/`error`)에 없는 메시지는 무시하지 않고 에러로 되돌아갑니다.
